...
{    QSqlQuery qry;
   ...
   qry.prepare("select * from client where sexe = 'FEMININ' ");
     if (qry.exec())
     {
         while (qry.next())
         {
     ...
     qry.prepare("select * from client where SEXE  = 'MASCULIN' ");
       if (qry.exec())
       {
           while (qry.next())
           {
    ...
    ui->customPlot->yAxis->setLabel("NUMBER OF CLIENTS");
